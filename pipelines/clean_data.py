            ...
            with open(json_file, 'r', encoding='utf-8') as f:
                ...
                if isinstance(file_data, list):
                    ...
                    for item in file_data:
                        item['_source_file'] = json_file.stem
                    data.extend(file_data)
                else:
                    file_data['_source_file'] = json_file.stem
...
def main():
    ...
    for i, page_data in enumerate(scraped_data):
        ...
        source_file = page_data.get('_source_file', 'unknown')
        
        # Check if this is English data format (has description field)
        if 'description' in page_data:
            url = f"english.uga.edu/{source_file}"
            title = "English Department Content"
            content = page_data.get('description', '')
        else:
            # CS data format (has url, title, body fields)
    ...
    for source, items in source_groups.items():
        # Save source-specific JSON
        source_json_path = output_dir / f"{source}_cleaned.json"
        with open(source_json_path, 'w', encoding='utf-8') as f:
            json.dump(items, f, indent=2, ensure_ascii=False)
        logger.info(f"Saved {source} cleaned data to {source_json_path}")
        
        # Save source-specific CSV
        source_df = pd.DataFrame(items)
        source_csv_path = output_dir / f"{source}_cleaned.csv"
        source_df.to_csv(source_csv_path, index=False)
        logger.info(f"Saved {source} cleaned data to {source_csv_path}")
